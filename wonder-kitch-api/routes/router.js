...
//Set Cooked Bollean True
router.post('/recipeCooked', function (req, res, next) {
  console.log(req.body)
  User.find({_id: req.body.userID}).then(function(user){
    let existItem=false;
   for (let i = 0; i < user[0].recipes.length; i++){
     console.log("recipeId is",user[0].recipes[i].recipeId, "cooked is", user[0].recipes[i].cooked)
     if(user[0].recipes[i].recipeId == req.body.recipeID){
       existItem=true;       
       const newData = User.updateOne({"recipes.recipeId": user[0].recipes[i].recipeId}, {$set: {"recipes.$.cooked":true}})
       return newData
     }      
   }  
 }).then(function(data){
